...
# Set up the logger first, before anything else
...
try:
    ...
    logger.error("Failed to import scapy_adapter. Attempting direct scapy import.")
    try:
        from scapy.all import Ether, ARP, IP, TCP, UDP, ICMP, Raw
        logger.info("Scapy module loaded directly")
    except ImportError:
        logger.critical("Failed to import scapy. Network simulation will not function properly.")
        # Define minimal mock classes to allow code to load
        class PacketMock:
        ...
        class Ether(PacketMock): pass
        class ARP(PacketMock): pass
        class IP(PacketMock): pass
        class TCP(PacketMock): pass
        class UDP(PacketMock): pass
        class ICMP(PacketMock): pass
        class Raw(PacketMock): pass
